import {FileInput, Label, Pagination, TextInput, Textarea } from 'flowbite-react'
...
export const CreateEventModule: React.FC = () => {
  ...
  const onSubmit = async (data: CreateEventForm) => {
    ...
    formData.append("start_date", new Date(data.start_date).toISOString().split('T')[0] || data.start_date);
    formData.append("end_date", new Date(data.end_date).toISOString().split('T')[0] || data.end_date);
          ...
          <form onSubmit={handleSubmit(onSubmit)} className='w-full'>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 justify-items-stretch w-full">
              ...
              <div className='col-span-2'>
                ...
                <p className='text-xs font-extralight'>Jelaskan apa saja yang perlu disiapkan peserta! <br/>Contoh: Payung, Sarung tangan, Masker</p>
              </div>
              <Textarea
                className='w-full col-span-3'
                rows={8}
              ...
              {loc?.lat && loc.lng ? (
                ...
                <DynamicMap 
                  ...
                  <p className='pt-1 font-extralight'>({loc.lat}, {loc.lng})</p>
              ...
              <div className='flex col-span-5 w-[100%] mx-auto justify-end gap-x-4'>
                <Button variant={'deserted'} ><h4>Batalkan</h4></Button>
                <Button variant={'greeny'}><h4>Ajukan</h4></Button>
              </div>
            </div>
          </form>
        </div>
